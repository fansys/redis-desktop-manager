...
set PYTHON_HOME=C:\Python37-x86
...
rem 执行依赖库引入
echo 执行依赖库引入
%QT_HOME%\msvc2017\bin\windeployqt --no-angle --no-opengl-sw --no-compiler-runtime --no-translations --release --force --qmldir %RDM_HOME%\src\qml rdm.exe
...
copy %PYTHON_HOME%\Lib\site-packages\lzf.cp37-win32.pyd .\Lib\site-packages\  /Y
...
rem 打包
echo 打包exe安装包
copy %HOME%\conf\installer_x86.nsi .\ /Y
"%NSIS_HOME%\makensis.exe" /V1 /DVERSION=%VERSION% ./installer_x86.nsi
del /s %PACKAGE_HOME%\redis-desktop-manager-2020.2.0_x86.exe
rename %PACKAGE_HOME%\redis-desktop-manager-2020.2.0.exe redis-desktop-manager-2020.2.0_x86.exe
